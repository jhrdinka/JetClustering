...
JetPlots::JetPlots(const TString name, const vector<float> ptvals) {
  ...
  for (vector<float>::const_iterator it = ptvals.begin();
       it != ptvals.end() - 1; it++) {
    float ptmin = *it;
    float ptmax = *(it + 1);
    TString ptbin_str;
    ...
    _ptbins.push_back(make_pair(make_pair(ptmin, ptmax),
                                TH1F(ptbin_str, ptbin_str, 500, 0., 4.0)));
  }
}

void JetPlots::fill(JetCollection& coll) const {
  for (unsigned i = 0; i < coll.size(); i++) {
    ...
    for (vector<pair<pair<float, float>, TH1F>>::iterator it = _ptbins.begin();
         it != _ptbins.end(); it++) {
...
void JetPlots::write() {
  ...
  for (vector<pair<pair<float, float>, TH1F>>::const_iterator it =
           _ptbins.begin();
       it != _ptbins.end(); it++) {
    (it->second).Write();
  }
